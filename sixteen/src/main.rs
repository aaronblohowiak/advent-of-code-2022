...
fn main() {
    ...
    let best = (done)
        .iter()
        .max_by(|a, b| a.pressure_released_so_far.cmp(&b.pressure_released_so_far));
...
fn load(fname: &str) -> Context {
    ...
    let res: Vec<(String, usize, Vec<String>)> =
    ...
    let successors =
        |&n: &usize| -> Vec<(usize, usize)> { input_map[&n].iter().map(|f| (*f, 1)).collect() };
